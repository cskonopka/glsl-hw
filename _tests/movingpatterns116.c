...
void main() {
    ...
    float y = (smoothstep((sin(+pct2+(sin(st.y+st.x))*(cos(u_time*0.13212))))*(tan((st.x)*00.20)),0.229005,0.32));
    // float y = (smoothstep((cos((sin(st.y))+u_time*0.3212))/(cos((st.x)*0.2301)),0.9005,(sin((st.y)*3.301))/sin((st.x)*.301)));

    float y2 = (smoothstep((sin(-pct2*(sin(st.y/st.x)*0.9591321)-(sin(.913212))))*(tan((st.x)*1.20)),0.229005,0.2832/cos(-st.y*st.x*0.020))) / (smoothstep((cos(-pct2*(sin(st.y*(st.x*0.02))*0.1321)-(tan(u_time*0.13212))))+(cos((st.x)*0.20)),0.229005,0.32));
