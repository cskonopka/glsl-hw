...
void main() {
    ...
    float y6 = (smoothstep(0.1,0.8915,-st.y*cos((-st.x*-st.y))) * smoothstep(0.4,0.091,st.y-st.x))*smoothstep(0.151,0.1915,st.y+cos(st.x*st.y)) - smoothstep(0.14,0.291,st.y-st.x);
    vec3 colorB = (vec3((y4+y5)*st.y+st.x))+(y3*(0.913)) * ((vec3(-y4-y5)*(-y3+(0.23+(st.y*st.x)))));
    colorB = (1.0)*colorB*vec3(3.1755,0.147,(-1.20+(y6+(st.y*st.x)))) - (1.0)*colorB*vec3(-0.215,-0.947,(0.120-(-y6-(st.y+st.x))));
