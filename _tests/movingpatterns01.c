...
void main() {
    ...
    float y3 = sin(cos((st.y)*(u_time*2.3)/18.0));
    float y = smoothstep(0.2,10.5,st.x) - smoothstep(0.5,0.1,(st.x*0.2));
    float y2 = smoothstep(0.1,0.15,st.y) - smoothstep(0.1,0.8,st.y);
    vec3 colorA = vec3(y+y2)+(y3*(u_time*0.3));
    colorA = (1.0)*colorA*vec3(1.0,1.0,(0.0+y3));
    ...
    colorMix = mix(colorA, colorB, 0.5);
