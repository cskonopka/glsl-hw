...
void main() {
    ...
    float y3 = sin(cos((st.y)*(0.10)))-sin(st.x*0.2);
    ...
    float y4 = cos(cos((st.x)*(u_time*0.13)*fract(0.3*u_time)));
    float y5 = smoothstep(0.2,0.5,st.y) - smoothstep(0.25,0.1+st.x,(st.y*0.12));
    float y6 = smoothstep(0.1,0.15,st.y+cos(st.y)) - smoothstep(0.41,0.1,st.x);
    ...
    colorMix = mix(colorA, colorB, (sin(u_time*((y6+y2)*0.2))*(cos(pct2*-2.2))));
