...
void main() {
    ...
    float y3 = sin(cos((st.y)*(0.10)))-sin(st.x*2.12);
    ...
    vec3 colorA = vec3(y*y2)+(y3*sin((u_time*0.13)));
    colorA = (1.0)*colorA*vec3(0.0,0.920,(0.0+(sin(y3+sin((u_time*10.2))))));
    ...
    vec3 colorB = vec3(y4+y5)+(y3*(sin(u_time*.13))) * vec3(y4*y5)+(y3+(u_time*-0.93));
    ...
    colorMix = mix(colorA, colorB, (sin(((y6)*.74))*(cos(pct2*0.92))));
