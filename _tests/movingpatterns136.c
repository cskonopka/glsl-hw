...
void main() {
    ...
    float y = (smoothstep((asin(-pct2*((sin(st.y+st.x))*0.231)*(cos(0.1913212))))*(tan((st.y)*1.20)),(0.229005),(0.132)))/(smoothstep((sin(pct2*((cos(st.y-st.x))*0.42001)*(cos(02.13212))))*(sin((st.x+st.y)*.20)),(0.229005),(.732*((cos(-st.y-st.x)*tan(20.93))/(cos(-st.x+st.y)*0.3)))));
    // float y = (smoothstep((cos((sin(st.y))+u_time*0.3212))/(cos((st.x)*0.2301)),0.9005,(sin((st.y)*3.301))/sin((st.x)*.301)));

    float y2 = (smoothstep((sin(-pct2*(sin(st.y/st.x)*1.519591321)-(sin(.9913212))))*(tan((-st.x+st.y)*0.20)),0.229005,0.2832/cos(-st.y*st.x*0.020))) / (smoothstep((cos(-pct2*(sin(st.y*(st.x*0.02))*0.1321)-(tan(0.91913212))))+(cos((st.y)*0.20)),0.9229005,0.132));
