...
void main() {
    ...
    colorA = (1.0)*colorA*vec3(.90,0.920,(100.0+(sin(y3+(u_time*0.12)))));

    float y4 = cos(cos((st.y-st.x)*(sin(u_time*0.1913))*cos(0.43*st.x+st.y)));
    float y5 = (smoothstep(0.1,0.295,st.x-st.y+cos(st.x*-st.y)));
    float y6 = (smoothstep(0.1,0.15,-st.y*cos((-st.x*-st.y))));
    vec3 colorB = (vec3((y4+y5)*st.y+st.x))+(y3*(0.13));
    ...
    colorMix = mix(colorA, colorB, ((sin(u_time*((-y6+y2*(st.y-st.x))*0.174))- sin(u_time*((y6-y2*(-st.y+st.x))*1.174)))));
