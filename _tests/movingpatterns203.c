...
void main() {
    ...
    float y2 = (smoothstep((-pct2*(sin(st.y/st.x*0.120)*.1519591321)+(sin(.19149913212)))/(tan((-st.x+st.y)*0.20)),0.9229005,0.212832*cos(sin(-st.y+st.x)*0.25020))) / (smoothstep((cos((sin(st.y*(st.x*0.02))*0.1321)+((tan(0.93913212))*cos(u_time)*0.2)))+(cos((st.y)*0.20)),st.y*0.19229005,(.91732*((sin(sin(-st.y-st.x)*u_time)*sin(0.1293))*(sin(-st.x+st.y)*0.13)))));
    ...
    colorB = (1.0)*colorB*vec3(.01289281990*(st.y+(cos(st.x)+cos(st.y*(0.082*cos(-st.y*sin(st.y)))))),0.01294120*sin((st.x*(st.y+(-0.92013+-st.y)))*0.9802),(0.084139230*(cos(y2+st.y*(01.97012))+(-st.y*st.x)))*(tan((1.0*(-st.y*-st.x)))*(.20*cos(-st.y+st.x))));
