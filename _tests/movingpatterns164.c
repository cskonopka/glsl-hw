...
void main() {
    ...
    float y2 = (smoothstep((-pct2*(sin(st.y/st.x*0.20)*.1519591321)+(sin(.149913212)))+(tan((-st.x+st.y)*0.20)),0.9229005,0.12832*cos(-st.y*st.x*0.25020))) / (smoothstep((cos((sin(st.y*(st.x*0.02))*0.1321)-(tan(0.1913212))))+(cos((st.y)*0.20)),st.y*0.19229005,(.1732*((cos(-st.y-st.x)*tan(0.293))+(cos(-st.x+st.y)*0.13)))));
    ...
    colorB = (1.0)*colorB*vec3(.1289281990*(st.y+(cos(st.x)+cos(st.y*(0.082*cos(-st.y*sin(st.y)))))),0.01294120*cos((st.x-(st.y-st.x*(-0.92013+-st.y)))*0.1802),(0.084139230*(cos(y2+st.y*(01.97012))+(-st.y*st.x)))*(tan((1.0*(-st.y*-st.x)))*(.20*cos(-st.y+st.x))));
