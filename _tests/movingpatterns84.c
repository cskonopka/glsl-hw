...
void main() {
    ...
    float y3 = sin(cos((st.y)*(5.10)))*sin(st.x*3.12) - tan(sin((st.y)*(-1.10)))*sin(tan(st.y)*st.x*1.12);
    float y = (smoothstep(.402,0.805,(st.y/st.x)));
    float y2 = (smoothstep(1.1,0.5,(st.y*st.x)*0.3232));
    // vec3 colorA = vec3(y*y2)+(y3*(u_time*0.013)) * vec3(-y*-y2)-(-y3*(u_time*-0.013));
    vec3 colorA = vec3(y+y2)+(y3*(cos(u_time*.0113)));
    colorA = (1.0)*colorA*vec3(.81990,0.420,(0.29230+(sin(y3*(u_time*0.7012)))));
